...
function bindIfParam(value, column) {
    if (value instanceof column_1.Column || value instanceof __1.Placeholder || column instanceof __1.SQL.Aliased) {
        return value;
    }
    else {
...
function inArray(column, values) {
    ...
    return (0, __1.sql) `${column} in ${bindIfParam(values, column)}`;
}
...
function notInArray(column, values) {
    ...
    if (Array.isArray(values)) {
        ...
        return (0, __1.sql) `${column} not in ${values.map((v) => bindIfParam(v, column))}`;
    }
    return (0, __1.sql) `${column} not in ${bindIfParam(values, column)}`;
}
...
function between(column, min, max) {
    return (0, __1.sql) `${column} between ${bindIfParam(min, column)} and ${bindIfParam(max, column)}`;
}
exports.between = between;
function notBetween(column, min, max) {
    return (0, __1.sql) `${column} not between ${bindIfParam(min, column)} and ${bindIfParam(max, column)}`;
}
